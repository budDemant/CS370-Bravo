...
class Chest(Cell):
    ...
    def on_collision(self, cell: "Cell") -> bool:
        if isinstance(cell, Player):
            ...
            if not self.has_paused_message:
                i = random.randint(2, game_instance.difficulty + 1)
                x = random.randint(2,4)
                game_instance.sm.current_state.pause_flash(10,25,f'You found {i} gems and {x} whips inside the chest!')
                self.has_paused_message = True
