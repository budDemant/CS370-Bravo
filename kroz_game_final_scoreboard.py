...
class CharTile(Tile):
    ...
    def render(self, surface: pygame.Surface, x: int, y: int, tile_size: int):
        pygame.draw.rect(surface, self.bg,
                        (x * tile_size, y * tile_size, tile_size, tile_size))
...
class Scoreboard(TileMap):
    ...
    def set_gems(self, new_gems: int):
        self.gems = new_gems
    ...
    def render(self, surface: pygame.Surface, tile_size: int):
        ...
        def put_nr(y: int, nr: int):
            val = str(nr).center(7)
            for i, c in enumerate(val):
                i += 3
                self.set_tile(i, y, WallTile())
                if c != " ":
                    self.set_text(c, (i, y), fg=COLORS['white'], bg=COLORS['brown'])


        self.set_text("Score", (4, 0), fg=COLORS['yellow'])
        put_nr(1, self.score)
        # score_value = str(self.score).center(7)
        # for i, c in enumerate(score_value):
        #     i += 3
        #     self.set_tile(i, 1, WallTile())
        #     if c != " ":
        #         self.set_text(c, (i, 1), fg=COLORS['white'], bg=COLORS['brown'])
        ...
        put_nr(7, self.gems)
        # score_value = str(self.gems).center(7)
        # for i, c in enumerate(score_value):
        #     i += 3
        #     self.set_tile(i, 7, WallTile())
        #     if c != " ":
        #         self.set_text(c, (i, 7), fg=COLORS['white'], bg=COLORS['brown'])
...
class KrozGame:
    ...
    def handle_input(self) -> bool:
        ...
        self.scoreboard.set_gems(self.gem)
    ...
    def draw(self):
        ...
        # Draw UI
        # font = pygame.font.Font(None, 36)
        # font = pygame.freetype.SysFont(["Perfect DOS VGA 437", "Courier New"], 36, bold=True)
        # # text_surface, _ = font.render(self.char, fgcolor=COLORS['white'], bgcolor=COLORS['brown'])
        #
        # score_text, _ = font.render(f'{self.score}', fgcolor=COLORS['white'])
        # # level_text = font.render(f'Level: {self.level}', True, COLORS['white'])
        # gem_text, _ = font.render(f'{self.gem}', fgcolor=COLORS['white'])
        #
